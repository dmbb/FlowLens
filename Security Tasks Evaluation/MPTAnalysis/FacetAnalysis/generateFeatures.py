...
def MergeDatasets(data_folder):
    ...
    features_files = [data_folder + "facet_dataset.csv", data_folder + "RegularTraffic_dataset.csv"]
...
def CombinedMerging(data_folder):
    ...
    features_files = [data_folder + "FacetTraffic_50_dataset.csv", data_folder + "RegularTraffic_dataset.csv"]
...
def FeatureExtractionPLBenchmark(sampleFolder, binWidth, topk):
    ...
    if(topk != 1500):
        ...
        f_imp = np.load('classificationResults/PL_60_' + str(binWidth) + '_1500/FeatureImportance_XGBoost_FacetTraffic_50.npy')
        #Print top k
        #for f in f_imp:
...
def CompressFeatures(BIN_WIDTH, TOPK):
    sampleFolders = [
    "TrafficCaptures/240Resolution/FacetTraffic_50",
    "TrafficCaptures/240Resolution/RegularTraffic",
    ]
...
def SplitDataset(DATASET_SPLIT, N_FLOWS, COVERT_FLOWS_PERC):
    ...
    for feature_folder in os.listdir("FeatureSets"):
        if(".DS_Store" not in feature_folder):
            ...
            #Split CovertFlows
            print "Splitting %s"%("FeatureSets/" + feature_folder + "/FacetTraffic_50_dataset.csv")
            CovertFile = open("FeatureSets/" + feature_folder + "/FacetTraffic_50_dataset.csv", "rb")
            csv_reader = csv.reader(CovertFile, delimiter=',')

            PhaseOneCovertFile = open("FeatureSets/" + feature_folder + "/FacetTraffic_50_phase1_dataset.csv", "w")
            PhaseTwoCovertFile = open("FeatureSets/" + feature_folder +  "/FacetTraffic_50_phase2_dataset.csv", "w")
...
def MergeTestData():
    for feature_folder in os.listdir("FeatureSets"):
        if(".DS_Store" not in feature_folder):
            ...
            print "Merging %s"%("FeatureSets/" + feature_folder + "/FacetTraffic_50_phase2_dataset.csv")
            ...
            PhaseTwoCovertFile = open("FeatureSets/" + feature_folder +  "/FacetTraffic_50_phase2_dataset.csv", "rb")
...
def FeatureExtractionPLBenchmarkBasedOnTrainData(sampleFolder, binWidth, topk):
    ...
    if(topk != 1500):
        #Buckets in decreasing importance order 
        f_imp = np.load('classificationResults/PL_60_' + str(binWidth) + '_1500/FeatureImportance_XGBoost_FacetTraffic_50_phase1.npy')
        #Print top k
        #for f in f_imp:
        #    print str(f[1]) + " " + str(f[2])
