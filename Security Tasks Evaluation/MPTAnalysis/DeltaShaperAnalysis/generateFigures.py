...
def PlotQuantization(binWidths, n_flows):
    ...
    plt.tight_layout()
    fig.savefig('Figures/DeltaShaper_bin_NoSketch.pdf')   # save the figure to file
    fig.savefig('Figures/DeltaShaper_bin_NoSketch.png')   # save the figure to file
    plt.close(fig)
...
def PlotQuantizationLines(binWidths, n_flows):
    ...
    for i,j in zip(ind,curr_acc):
        ax1.annotate("{0:.2f}".format(j),xy=(i-0.15,j+0.03))
    ...
    plt.xlabel("DeltaShaper Quantization Factor (K)", fontsize=12)
    ax1.legend()

    
    plt.legend(loc='lower right', fontsize=12)
    plt.tight_layout()
    fig.savefig('Figures/DeltaShaper_bin_NoSketch_Lines.pdf')   # save the figure to file
    fig.savefig('Figures/DeltaShaper_bin_NoSketch_Lines.png')   # save the figure to file
    plt.close(fig)
...
def PlotKQuantizationAndTruncation(binWidths, topk_features, n_flows):
    ...
    for binWidth in binWidths:
        ...
        plt.tight_layout()
        fig.savefig('Figures/Truncation_comparison/DeltaShaper_bin' + str(binWidth) + '_topk_NoSketch.pdf')   # save the figure to file
        fig.savefig('Figures/Truncation_comparison/DeltaShaper_bin' + str(binWidth) + '_topk_NoSketch.png')   # save the figure to file
        plt.close(fig)
...
def PlotKQuantizationAndTruncationLines(binWidths, topk_features, n_flows):
    ...
    for binWidth in binWidths:
        ...
        for i,j in zip(ind,curr_acc):
            ax1.annotate("{0:.2f}".format(j),xy=(i-0.1,j+0.03))
        ...
        plt.legend(loc='lower right', fontsize=12)
        plt.tight_layout()
        fig.savefig('Figures/Truncation_comparison/DeltaShaper_bin' + str(binWidth) + '_topk_NoSketch_Lines.pdf')   # save the figure to file
        fig.savefig('Figures/Truncation_comparison/DeltaShaper_bin' + str(binWidth) + '_topk_NoSketch_Lines.png')   # save the figure to file
        plt.close(fig)
...
def GenerateFiguresLines(binWidths, topk_features, nFlows):
    if not os.path.exists('Figures'):
        os.makedirs('Figures')
